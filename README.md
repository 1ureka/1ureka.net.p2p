...
在一次完整的連線過程中，資料會依序經過不同模組，並隨著不同階段的事件轉換而流動。

當應用想要建立 TCP 連線時，Client Adapter 會先把請求封裝後送出；Host Adapter 驗證規則後再嘗試連接服務。連線成功後，雙方便能透過資料封包雙向傳遞內容；最後當任一端結束，則會送出關閉訊號，並釋放相關資源：
